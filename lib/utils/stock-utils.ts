...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://api.brandonling.net/api";
...
async function getQuickPrice(ticker: string) {
  try {
    ...
    console.error(`Failed to get quick price for ${ticker}:`, error);
    ...
    return mockPrices[ticker] || Math.random() * 200 + 50;
  }
}

async function getRealTimePrice(ticker: string) {
  try {
    // Use Next.js API route for real-time price
    const response = await fetch(`/api/stocks?ticker=${ticker}&realtime=true`);
    if (!response.ok) throw new Error('API route failed');
    const data = await response.json();
    return Number(data.price);
  } catch (error) {
    console.error(`Failed to get real-time price for ${ticker}:`, error);
    // Fallback to quick price
    return getQuickPrice(ticker);
  }
}
...
export async function getStockPrice(
  ...
  try {
    ...
    if (cacheError) {
      ...
      const cacheAge =
        (Date.now() - new Date(cachedPrice.timestamp).getTime()) / 1000;
    ...
    console.error(
      "Failed to get quick price, falling back to historical:",
      error
    );
...
export async function getHistoricalPrices(
  ...
  if (cachedPrices) {
    // && cachedPrices.length >= expectedDays * 0.9
  ...
  if (priceRecords.length > 0) {
    await supabase.from("stock_price_history").upsert(priceRecords, {
