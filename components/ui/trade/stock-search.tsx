...
export default function StockSearch({
  ...
  useEffect(() => {
    const searchStocks = async () => {
      ...
      try {
        ...
        const mappedResults = (data || []).slice(0, 4).map((stock: any) => ({
          symbol: stock.ticker || stock.symbol,
          name: stock.name || stock.ticker,
          type: stock.type || 'Stock',
          exchangeShortName: stock.exchangeShortName || 'NASDAQ'
        }));
  ...
  const estimatedTotal = currentPrice
    ? Number(quantity) * currentPrice + commission
    : 0;
    ...
    <div className="space-y-4">
      <div className="space-y-6">
        ...
        <div>
          ...
          <Select
            value={action}
            onValueChange={(value) => onActionChange(value as "buy" | "sell")}
          >
            <SelectTrigger>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <Button variant="outline" onClick={handleShowMax}>
              Show Max
            </Button>
          </div>
        </div>
