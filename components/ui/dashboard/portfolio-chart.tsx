    ...
    const fetchPortfolioHistory = async () => {
      try {
        ...
        if (historyError) {
          ...
          setError(
            `Failed to fetch portfolio history: ${historyError.message}`
          );
          return;
        }

        const expectedDays = getMarketDays(
          startDate,
          new Date(),
          holidays
        ).length;
        const hasEnoughData =
          historyData && historyData.length >= expectedDays * 0.9;

        if (!error && hasEnoughData) {
          // filter out any holiday dates from the history data
          const filteredData = historyData.filter(
            (record) => !holidays.has(formatDateStr(new Date(record.timestamp)))
          );
        ...
        const calculatedData = await Promise.all(
          daysToCalculate.map(async (dateStr) => {
            const transactionsToDate = transactions.filter(
              (t) => formatDateStr(new Date(t.executed_at)) <= dateStr
            );
                ...
                ticks={
                  chartData.length > 0
                    ? (() => {
                        ...
                        const step = Math.max(1000, Math.round((max - min) / 5));
                        const ticks = [];
                        ...
                        // Ensure we don't have duplicates and have max value
                        const uniqueTicks = [...new Set(ticks)];
                        if (!uniqueTicks.includes(max)) {
                          uniqueTicks.push(max);
                        }
