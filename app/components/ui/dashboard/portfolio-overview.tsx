  ...
  const cards = [
    ...
    },
    {
      ...
      value: dailyChange !== null 
        ...
        : 'N/A',
      change: dailyChange !== null ? dailyChangePercent : null,
      prefix: dailyChange !== null ? (dailyChange >= 0 ? '+' : '-') : ''
    }
  ];
      ...
      {cards.map((card, i) => (
        <Card key={i} className="p-6">
          ...
          {card.change !== undefined && (
            <div className={`text-sm mt-1 ${card.change !== null && (card.change >= 0 ? 'text-green-500' : 'text-red-500')}`}>
              {card.change !== null && (card.change >= 0 ? '↑' : '↓')} {card.change !== null ? Math.abs(card.change).toFixed(2) : 0}%
              {card.changeValue && (
                <span className="ml-1">
