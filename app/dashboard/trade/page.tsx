...
import { getStockPrice, isMarketOpen } from "@/lib/utils/stock-utils";
import { createClient } from "@/lib/utils/supabase/client";
import { processPendingTrades } from "@/lib/utils/trades";
...
export default function TradePage() {
  ...
  const validateShares = async (shares: number) => {
    ...
    try {
      if (orderType === "sell") {
        ...
        const totalPendingSells =
          pendingSells?.reduce((sum, trade) => sum + trade.quantity, 0) || 0;
        ...
        const reservedFunds =
          pendingBuys?.reduce(
            (sum, trade) =>
              sum + trade.quantity * trade.price + trade.commission,
            0
          ) || 0;
  ...
  const handleSymbolSelect = async (symbol: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch stock price"
      );
      setPrice(null);
    } finally {
      setLoading(false);
  ...
  const handleShowMax = async () => {
    if (!ticker || ticker.trim() === "") {
      return {
        error: "Please enter a valid ticker symbol first",
        maxQuantity: 0,
      };
    }
    ...
    if (!price || isNaN(price) || price <= 0) {
      return {
        error: "Unable to fetch current price or price is invalid",
        maxQuantity: 0,
      };
    }
    ...
    try {
      if (orderType === "sell") {
        ...
        if (pendingError) {
          return {
            error: "Unable to calculate available shares",
            maxQuantity: 0,
          };
        }
        ...
        const availableShares = Math.max(
          0,
          holding.quantity - totalPendingSells
        );

        if (availableShares === 0) {
          return {
            error: "No shares available to sell after pending orders",
            maxQuantity: 0,
          };
        }
        ...
        if (pendingError) {
          return {
            error: "Unable to calculate available funds",
            maxQuantity: 0,
          };
        }
        ...
        if (maxShares === 0) {
          return {
            error: "Insufficient funds to buy any shares",
            maxQuantity: 0,
          };
        }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      {marketClosed && (
        <div className="mb-4 p-3 bg-red-500/10 text-red-500 rounded-lg">
          Market is closed. Trading hours are 9:30 AM - 4:00 PM ET,
          Monday-Friday
        </div>
      )}
